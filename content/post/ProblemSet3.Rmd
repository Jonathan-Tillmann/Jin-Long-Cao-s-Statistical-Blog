...
gss %>%  
ggplot(aes(x = average_hours_worked)) +
...
```
In figures ?????? and ?????? we can see the occupation demographics for male and female respondents respectively. Statistics Canada grouped occupations into sectors and we used pie charts to easily display the differences in occupation sector percentages by gender.
...
gss %>% 
  ggplot(aes(fill=sex, x=marital_status)) +
  geom_bar()+
  theme(axis.text.x=element_text(angle=90),panel.background = element_rect(fill = "lightblue", colour = "lightblue", size = 0.5, linetype = "solid")) +
  labs(x="Respondent's Marital Status",
       y="Frequency",
       title="Respondent's Marital Status")
...
gss %>% 
  ...
  labs(x="Respondent's Average Hours Worked",
       y="Frequency",
       title="Respondent's Average Hours Worked")
...
```{r}
mod<-glm(gss$sex~income_respondent_rank+avg_hrs_worked_rank+gss$occupation+gss$education, family="binomial", na.action=na.omit)
summary(mod)
plot(mod)
```
## Discussion
